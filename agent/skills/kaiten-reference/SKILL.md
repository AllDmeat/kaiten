...
The `reference` key in the CLI config (`~/.config/kaiten/config.json`, or the file passed via
`--config`) points at it, and that key is the only place to look — do not go hunting for
reference-looking files elsewhere. The path can be anything; when suggesting one, suggest the
...
resolve an ID through the API and the config has no `reference` key, tell the user what you had
to look up and offer to start the file with it. Propose the default path,
`~/.config/kaiten/reference.md`, unless the user prefers another path or format, and write the
`reference` key into the config either way — the key is how every future session finds the file.
Seed it only with IDs the user confirms — do not bulk-dump the API into it: a reference full of
unverified entries, duplicates included, defeats its purpose.
